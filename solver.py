...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            out_src = self.D(mc_real, spk_c_trg, spk_c_org)                      ########################## check it###################################
            ...
            mc_fake = self.G(mc_real, spk_c_org, spk_c_trg)
            #mc_fake_real = self.G(mc_fake, spk_c_org)

            out_src = self.D(mc_fake.detach(), spk_c_org, spk_c_trg)
            ...
            out_src = self.D(x_hat, spk_c_org, spk_c_trg)
            ...
            if (i+1) % self.n_critic == 0:
                ...
                mc_fake = self.G(mc_real, spk_c_org, spk_c_trg)
               # mc_fake_real = self.G(mc_fake, spk_c_org)
                out_src = self.D(mc_fake, spk_c_org, spk_c_trg)
                g_loss_fake = - torch.mean(out_src)

                # Target-to-original domain.
                mc_reconst = self.G(mc_fake, spk_c_trg, spk_c_org)
                g_loss_rec = torch.mean(torch.abs(mc_real - mc_reconst))

                # Original-to-Original domain(identity).
                mc_reconst_id = self.G(mc_real, spk_c_org, spk_c_org)
                g_loss_id_rec = torch.mean(torch.abs(mc_real - mc_reconst_id))

                # Backward and optimize.
                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_rec * g_loss_id_rec
                self.reset_grad()
                g_loss.backward()
            ...
            if (i+1) % self.sample_step == 0:
                ...
                with torch.no_grad():
                    for idx, wav in tqdm(enumerate(test_wavs)):
                        ...
                        conds_trg = torch.FloatTensor(self.test_loader.spk_c_trg).to(self.device)
                        conds_org = torch.FloatTensor(self.test_loader.spk_c_org).to(self.device)
                        # print(conds.size())
                        coded_sp_converted_norm = self.G(coded_sp_norm_tensor, conds_org, conds_trg).data.cpu().numpy()
